...
WBDatatablesClass.prototype = {
    ...
    showThumbnail: function (thumbnail) {
        if (!thumbnail) {
            thumbnail = base_domain+'/public/assets/obn-dashboard/img/no-image.png';
        }
    ...
    showPublish: function (is_published, published_at, route_update, deleted_at = '', deleted_by = '') {
        var label_status = '';
        var checked = '';
        ...
        if(deleted_by != '') {
            ...
            html += '<div><a class="published_at text-muted text-size-small" data-field="published_at" data-action="' + route_update + '">' + published_at + '</a></div>';
        }
        return html;
    },
