...
@section('content')
    <div class="panel panel-flat">
        <table class="table table-xlg datatable-ajax" data-source="{{ url('public/data/product.json') }}"
            data-destroymulti="https://dainghiagroup.com/admin/product/destroy-multi"
            data-updatemulti="https://dainghiagroup.com/admin/product/update-multi">
            <thead>
                <tr>
                    ...
                    <th width="60" class="text-center">Sắp xếp</th>
...
<script type="text/javascript">
    if($('.datatable-ajax').length){
        var columnDatas = [
            ...
            },
            {
                ...
                render: function (data) {
                    return WBDatatables.showTitle(data.description.title, data.route_edit, data.is_published, data.published_at);
                },
                orderable: false, searchable: false
            },
            {
            ...
            },
            {
                ...
                class: "text-right",
                render: function (data) {
                    var $element = `<a href="javascript:void(0);" class="price" data-popup="popover-price" data-id="${data.id}" data-url="https://dainghiagroup.com/admin/product/${data.id}/update-price" data-price="${data.price}" data-price_sale="${data.sale_price }">
                        ${data.sale_price_formated}`;
            ...
            },
            {
                ...
                render: function (data) {
                    if (data.is_advanced_quantity) {
                        return `<a href="javascript:void(0);" data-popup="popover-quantity" class="${data.quantity > 0 ? 'text-success' : 'text-danger'}" data-id="${data.id}" data-checked="${data.allow_out_of_stock_order}" data-quantity="${data.quantity}" data-url="https://dainghiagroup.com/admin/product/${data.id}/update-field">
                                ${data.quantity > 0 ? 'Còn hàng' : 'Hết hàng'}
                            </a>`;
                    } else {
                        return `<a href="javascript:void(0);" class="switchery-checkbox ${data.in_stock == 1 ? 'text-success' : 'text-danger'}" data-type="checklist" data-pk="${data.id}" data-title="Đổi trạng thái" data-value="${data.in_stock}" data-url="https://dainghiagroup.com/admin/product/${data.id}/update-field">
            ...
            },
            {
                ...
                name: "published_at",
                render: function (data) {
                    return WBDatatables.showPublish(data.is_published, data.published_at, data.route_update, data.deleted_at, data.deleted_by)
                },
            ...
            $('.switchery-checkbox').editable({
                ...
                success: function (response, newValue) {
                    ...
                    if (response.success && newValue.length) {
                        $('table ').find(`a[data-pk='${response.id}']`).removeClass('text-danger').addClass('text-success');
                    } else {
                        $('table').find(`a[data-pk='${response.id}']`).removeClass('text-success').addClass('text-danger');
                    }
                }
            });
        ...
        let renderChangePricePopup = () => {
            $('[data-popup="popover-price"]').popover({
                ...
                content: function () {
                    var data = $(this).data();
                    var $form_price = '<form id="form_price_' + data.id + '" class="wb_ajax_submit" method="POST" action="' + data.url + '">' +
                        ...
                        '  <input type="number" class="form-control" value="' + data.price_sale + '" name="price_sale">' +
                        '</div>' +
                        '<div class="editable-buttons editable-buttons-bottom">' +
                        '<button type="button" class="btn btn-primary btn-sm button_submit_form" data-form="form_price_' + data.id + '"><i class="glyphicon glyphicon-ok"></i></button>' +
        ...
        let renderChangeQuantity = () => {
            $('[data-popup="popover-quantity"]').popover({
                ...
                content: function () {
                    let data = $(this).data();
                    let html = '<form id="form_quantity_' + data.id + '" class="wb_ajax_submit" method="POST" action="' + data.url + '">' +
                        '<input type="hidden" name="_token" value="ANv3sHccUyxwPMnYXlScYxEM3yjAnvgzZBOSE6Ex">' +
                        ...
                        '</div>' +
                        '<div class="editable-buttons editable-buttons-bottom">' +
                        '<button type="button" class="btn btn-primary btn-sm button_submit_form" data-form="form_quantity_' + data.id + '"><i class="glyphicon glyphicon-ok"></i></button>' +
                        '</form>';
                    return html;
                }
            });
        ...
        var is_published = `<select class="form-control" name="is_published"><option value="-1" selected="selected">Tất cả trạng thái</option><option value="0">Ẩn</option><option value="1">Hiện</option><option value="2">Đang chờ hiện</option><option value="-2">Thùng rác</option></select>`;
    ...
    let html = '<div class="dataTables_filter"><a class="btn btn-default button-export" target="_blank" href="'+base_domain+'/admin/product/export" role="button">Xuất danh sách đã lọc</a></div>';
